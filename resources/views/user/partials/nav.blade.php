<ul id="main-nav" class="nav rad tmnf_custom_menu"><li id="menu-item-5098" class="menu-item menu-item-type-post_type menu-item-object-page"><a href="">Home</a></li>

<li id="menu-item-5677" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a href="#">About</a>
<ul class="sub-menu">
    <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.contacts')}}">Contact</a></li>
    
    
    
</ul>

</li>

<li id="menu-item-5677" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a href="#">Apply For Case</a>
<ul class="sub-menu">
    <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.nidds')}}">Apply</a></li>
    
   
</ul>
<li id="menu-item-5677" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a href="#">Registration Form</a>
<ul class="sub-menu">
    <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.registration')}}">Apply</a></li>
    
   
</ul>
<li id="menu-item-5677" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a href="#">Feedback</a>
<ul class="sub-menu">
    <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.feedbacks')}}">View</a></li>
    
   
</ul>
<li id="menu-item-5677" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a href="#">Office <br>Services</a>
<ul class="sub-menu">
   <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.healthcare')}}">Health care</a></li>
   <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.law')}}">Law and order</a></li>
   <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.travel')}}">Travel</a></li>
   <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.recruitment')}}">Recruitment related</a></li>
   <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.education')}}">Education</a></li>

   
</ul>
<li id="menu-item-5677" class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children"><a href="#">Notice</a>
<ul class="sub-menu">
    <li id="menu-item-5786" class="menu-item menu-item-type-custom menu-item-object-custom"><a href="{{route('user.notice')}}">View</a></li>
    
   
</ul>

</li>
</ul>


